...
module.exports = function (options) {
  ...
  function transformer(tree) {
    if (settings.after || settings.after === 0) {
      findAllAfter(tree, getNode(tree, settings.after), function (n) {
        return transform(n);
      });
    } else if (settings.before) {
      findAllBefore(tree, getNode(tree, settings.before), function (n) {
        return transform(n);
      });
    } else if (settings.between) {
      findAllBetween(tree, tree.children.indexOf(getNode(tree, settings.between[0])) + 1, getNode(tree, settings.between[1]), function (n) {
        return transform(n);
      });
    } else {
